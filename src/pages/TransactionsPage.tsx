  ...
  const monthlyRows = useMemo(() => {
    ...
    for (let m = 12; m >= 1; m--) {
      ...
      const hasTransactions = yearTransactions.some(
        (t) =>
          t.transaction_date >= cycle.start && t.transaction_date <= cycle.end,
      );
                    ...
                    <div key={date} className="glass rounded-2xl overflow-hidden">
                      <div className="flex items-center gap-3 px-4 py-3.5 border-b border-dark-700/50">
                        <div className="w-14 h-14 rounded-2xl bg-primary-500/15 border border-primary-500/25 flex items-center justify-center shrink-0">
                          <span className="text-3xl font-bold text-dark-100 leading-none">{dayNum}</span>
                        </div>
                        <div className="flex flex-col gap-0.5">
                          <span className="inline-flex items-center justify-center px-2 py-0.5 rounded-md bg-dark-800 text-[10px] font-semibold text-primary-300 w-fit">{dayName}</span>
                          <span className="text-xs text-dark-400">{monthYear}</span>
                          <span className="text-[10px] text-dark-500">{txs.length} transaksi</span>
                        </div>
                        <div className="flex-1" />
