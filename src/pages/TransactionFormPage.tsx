...
export function TransactionFormPage() {
  ...
  const editData = (location.state as any)?.editData as TransactionWithDetails | null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (type === 'transfer') {
      ...
      result = await addTransfer(data.from_account_id, data.to_account_id, data.amount, data.transaction_date, data.note || undefined);
    } else {
      const data = {
        amount: numAmount,
  ...
  return (
    <div className="max-w-lg mx-auto bg-dark-900 min-h-screen text-dark-100 flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-4 border-b border-dark-800 sticky top-0 bg-dark-900/95 backdrop-blur-sm z-10">
        ...
        <h1 className="font-medium text-[15px]">{editData ? 'Edit Transaksi' : 'Tambah Transaksi'}</h1>
